...
import "./interfaces/IPDBs.sol";
import "./interfaces/IPDBsFactory.sol";

contract PDBsMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable,
    IERC721Receiver,
    UUPSUpgradeable
{
    IPDBsFactory private PDBsFactory;
    ...
    function initialize(
        ...
        PDBsFactory = _PDBsFactory;
    }

    modifier isPDBsNFT(address _nft) {
        require(PDBsFactory.isPDBsNFT(_nft), "not PDBs NFT");
        _;
    }
    ...
    function completeBid(address _nft, uint256 _tokenId) external {
        ...
        IPDBs PDBsNft = IPDBs(_nft);
        address royaltyRecipient = PDBsNft.getRoyaltyRecipient();
        uint256 royaltyFee = PDBsNft.getRoyaltyFee();
